...
## Audio Transform
The audio tranform directory houses the scripts that
transorm audio into spectrograms. Audio comes in
in the .dat file format, is translated to .wav format,
then transformed into spectrograms.

## Image Inference
The image inference directory houses the scripts that
run inference on spectrograms and packetize
...
preferably yolo8 or yolo11.  

## Sys Control
The system control directory houses the scripts that start the detector,
select the audio files for analysis, call the translation and 
transformation scripts, and handle life cycle. This code 
operates the system. 
